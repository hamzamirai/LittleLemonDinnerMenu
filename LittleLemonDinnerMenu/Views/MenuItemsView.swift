        ...
        NavigationView {
            ...
            .toolbar {
                ToolbarItem(placement: .navigationBarTrailing) {
                    Button(
                        action: {
                        // Trigger the sheet to show
                        isShowingOptionsSheet = true
                    }) {
                        Image(systemName: "slider.horizontal.3")
                    }
                }
            }
            .sheet(isPresented: $isShowingOptionsSheet) {
                MenuItemsOptionView(viewModel: viewModel)
            }
            .onAppear {
                // Ensures menu items are updated when the view appears
    ...
    private func filterIndicatorView() -> some View {
        ...
        case .alphabetical:
            ...
            Text(String(item.title.prefix(1)))
                ...
                .cornerRadius(8)
        case .none:
            // If no sort option is selected, do not show the indicator
            EmptyView()
        }
    }
}
